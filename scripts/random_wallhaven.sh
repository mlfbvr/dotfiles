...
RESOLUTIONS="${WALLHAVEN_RESOLUTIONS:-}"
...
if [ -n "$RESOLUTIONS" ]; then
    IMG_URL="${IMG_URL}resolutions=${RESOLUTIONS}&"
fi
...
IMG_URL="${IMG_URL}&categories=${CATEGORIES}&sorting=${SORT}&purity=${PURITY}"

#${CURL} -s ${IMG_URL} | ${JQ} .data[${INDEX}].path | xargs feh #--bg-fill 
IMG_PATH=$(${CURL} -s ${IMG_URL} | ${JQ} -r .data[${INDEX}].path)
